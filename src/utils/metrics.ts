...
/**
 * Round to 15 decimal places for precision in calculations
 */
...
export function diffArray(arr: number[], order: number = 1): number[] {
  ...
  for (let i = order; i < arr.length; i++) {
    diff.push(toPrecise(arr[i] - arr[i - order]))
  }
  return diff
}
...
export function movingAvg(arr: number[], window: number): number[] {
  ...
  for (let i = 0; i <= arr.length - window; i++) {
    ...
    result.push(isFinite(avg) ? toPrecise(avg) : 0)
  }
  return result
}
...
export function meanDuplicate(duplicates: number[][]): number[] {
  ...
  for (let i = 0; i < length; i++) {
    ...
    result.push(isFinite(mean) ? toPrecise(mean) : 0)
  }
...
export function subtractArray(data: number[], bgCtrl: number[]): number[] {
  ...
  for (let i = 0; i < minLength; i++) {
    ...
    result.push(isFinite(diff) ? toPrecise(diff) : 0)
  }
...
export function normaliseAlexa(data: number[][], alexa0: number, alexa100: number): number[] {
  ...
  return mean.map(value => {
    ...
    return isFinite(normalized) ? toPrecise(normalized) : 0
  })
}
...
export function calcHoFFWithDebug(options: {
  ...
  // Find HLT
  const rawHltIndex = debug.normalizedValues.findIndex((v) => v >= hltPercentage)
  ...
  debug.mlr = toPrecise(Math.max(...debug.smoothedValues.filter(isFinite)))
  if (!isFinite(debug.mlr)) debug.mlr = 0
  
  // Convert 0-based index to 1-based time point (0-59min → 1-60min)
  const rawTmlrIndex = debug.smoothedValues.indexOf(Math.max(...debug.smoothedValues.filter(isFinite)))
  debug.tmlr = rawTmlrIndex >= 0 ? rawTmlrIndex + 1 : -1
  const fi = debug.tmlr > 0 ? toPrecise(debug.mlr / debug.tmlr) : 0
...
export function calcHoFF(options: {
  ...
  const hlt = norm.findIndex((v) => v >= hltPercentage)
  ...
  const mlr = toPrecise(Math.max(...smooth))
  if (!isFinite(mlr)) return 0
  
  // Convert 0-based index to 1-based time point (0-59min → 1-60min)
  const rawTmlrIndex = smooth.indexOf(Math.max(...smooth))
  const tmlr = rawTmlrIndex >= 0 ? rawTmlrIndex + 1 : -1
  const fi = tmlr > 0 ? toPrecise(mlr / tmlr) : 0
